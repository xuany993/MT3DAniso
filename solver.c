...
void Boundary_E0(int edge_ind,complex double *E0)
{
     ...
     switch (ZDirection)
     {
        case 1:
          if (depth>0)
          {
               *E0  = A0*cexp((-1.0)*k0*(layer_depth[1]-depth))+\
               B0*cexp(k0*(layer_depth[1]-depth));                              // z轴竖直向上,layer_depth[1] 表示空气层厚度
          }
          else if(depth<=0)
          {
        ...
        case 2:
          ...
          {
               *E0  = A0*cexp((-1.0)*k0*(layer_depth[1]+depth))+\
               B0*cexp(k0*(layer_depth[1]+depth));                               // z轴竖直向下,layer_depth[0] 表示空气层厚度 1108
          }
          else if(depth>=0)
          {
               *E0  = A1*cexp((-1.0)*k1*(layer_depth[1]+depth));
          }
          break;
     }
