...
struct Anisotropic
{
    int AnisoNumber;  //各向异性的区域个数 
    int*  areaindex;  //各项异性区域的编号
    double* value;    //存储主轴各项异性值(按照sigma1,sigma2,sigma3;sigma1,sigma2,sigma3...顺序存储)
    double* angle;    //存储欧拉旋转角(按照走向角,倾角,倾向角;走向角,倾角,倾向角...顺序存储)
};
...
extern int     ZDirection;

extern int     phy_num;
extern double *phy_cond;
extern int     phy_epsl_num;
extern double *phy_epsl;
extern int     phy_mu_num;
extern double *phy_mu;
...
//--------Anisotropic-------------
extern struct Anisotropic anisoc;//电导率各向异性
extern struct Anisotropic anisom;//磁导率各向异性
extern struct Anisotropic anisod;//介电常数各向异性
