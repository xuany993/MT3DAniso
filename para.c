...
void Data_distribute()
{
     ...
     if (myrank==0)
     {
          ...
          //added by zhengxy
          par_list[19]=phy_mu_num;
          par_list[20]=anisoc.AnisoNumber;
          par_list[21]=anisom.AnisoNumber;
          par_list[22]=ZDirection;

     }

     MPI_Bcast(par_list,23,MPI_INT,0,MPI_COMM_WORLD);
     ...
     if (myrank!=0)
     {
          ...
          //added by zhengxy
          phy_mu_num=par_list[19];
          anisoc.AnisoNumber=par_list[20];
          anisom.AnisoNumber=par_list[21];
          ZDirection=par_list[22];
          ...
          //added by zhengxy
          phy_mu  =(double*)malloc((phy_mu_num+1)*sizeof(double));
          anisoc.areaindex = (int*)malloc((anisoc.AnisoNumber+1)*sizeof(int));
          anisoc.angle = (double*)malloc((anisoc.AnisoNumber*3+1)*sizeof(double));
          anisoc.value = (double*)malloc((anisoc.AnisoNumber*3+1)*sizeof(double));
          anisom.areaindex = (int*)malloc((anisom.AnisoNumber+1)*sizeof(int));
          anisom.angle = (double*)malloc((anisom.AnisoNumber*3+1)*sizeof(double));
          anisom.value = (double*)malloc((anisom.AnisoNumber*3+1)*sizeof(double));
          
     }

     l_temp_d= node_number_total*3+phy_num+fre_num+collect_num*3+layer*2+phy_mu_num+anisoc.AnisoNumber*6+anisom.AnisoNumber*6+1;
     l_temp_i= elem_number_total*5+anisoc.AnisoNumber+anisom.AnisoNumber+1;
     ...
     if (myrank==0)
     {
          ...
          //added by zhengxy
          temp=temp+layer;
          for (i=1;i<=phy_mu_num;i++)
               temp_d[temp+i]=phy_mu[i];
          temp=temp+phy_mu_num;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.value[3*(i-1)+1];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.value[3*(i-1)+2];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.value[3*(i-1)+3];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.angle[3*(i-1)+1];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.angle[3*(i-1)+2];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               temp_d[temp+i]=anisoc.angle[3*(i-1)+3];
          temp=temp+anisoc.AnisoNumber;

          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.value[3*(i-1)+1];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.value[3*(i-1)+2];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.value[3*(i-1)+3];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.angle[3*(i-1)+1];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.angle[3*(i-1)+2];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               temp_d[temp+i]=anisom.angle[3*(i-1)+3];
          ...
          //added by zhengxy
          temp=temp+elem_number_total;
          for (i=1;i<=anisoc.AnisoNumber;i++) temp_i[i+temp]=anisoc.areaindex[i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++) temp_i[i+temp]=anisom.areaindex[i];
     }
     ...
     if (myrank!=0)
     {
          ...
          temp=temp+layer;
          for (i=1;i<=phy_mu_num;i++)
               phy_mu[i]=temp_d[temp+i];
          temp=temp+phy_mu_num;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.value[3*(i-1)+1]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.value[3*(i-1)+2]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.value[3*(i-1)+3]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.angle[3*(i-1)+1]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.angle[3*(i-1)+2]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisoc.AnisoNumber;i++)
               anisoc.angle[3*(i-1)+3]=temp_d[temp+i];
          temp=temp+anisoc.AnisoNumber;

          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.value[3*(i-1)+1]=temp_d[temp+i];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.value[3*(i-1)+2]=temp_d[temp+i];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.value[3*(i-1)+3]=temp_d[temp+i];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.angle[3*(i-1)+1]=temp_d[temp+i];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.angle[3*(i-1)+2]=temp_d[temp+i];
          temp=temp+anisom.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++)
               anisom.angle[3*(i-1)+3]=temp_d[temp+i];
          ...
          //added by zhengxy
          temp=temp+elem_number_total;
          for (i=1;i<=anisoc.AnisoNumber;i++) anisoc.areaindex[i]=temp_i[i+temp];
          temp=temp+anisoc.AnisoNumber;
          for (i=1;i<=anisom.AnisoNumber;i++) anisom.areaindex[i]=temp_i[i+temp];
     }
